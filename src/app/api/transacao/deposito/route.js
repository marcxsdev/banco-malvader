...
export async function POST(request) {
  try {
    ...
    const transacao = await createTransacao({
      ...
      valor: valorNumerico,
      descricao: `Depósito de R$${valorNumerico.toFixed(2)}`,
    });
    ...
    await logAuditoria(
      id_usuario,
      "DEPOSITO",
      `Sucesso: depósito de R$${valorNumerico.toFixed(
        2
      )} na conta ${numero_conta}`
    );
    ...
    await logAuditoria(null, "DEPOSITO", `Erro: ${error.message}`);
    if (error.message.includes("Limite diário de depósito excedido")) {
      return NextResponse.json(
        { error: "Limite diário de depósito excedido" },
        { status: 400 }
      );
    }
    return NextResponse.json(
